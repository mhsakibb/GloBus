...
const Header = () => {
  ...
  const [menuOverflow, setMenuOverflow] = useState("hidden");
  ...
  // Handle mobile menu overflow timing for smooth animations
  useEffect(() => {
    if (mobileMenuOpen) {
      const timer = setTimeout(() => setMenuOverflow("visible"), 100);
      return () => clearTimeout(timer);
    } else {
      setMenuOverflow("hidden");
    }
  }, [mobileMenuOpen]);
  ...
  return (
    <>
      <header className="bg-gray-900 sticky z-50 top-0 shadow-lg border-b border-gray-700 w-full">
        <div className="flex flex-wrap md:flex-nowrap items-center justify-between mx-2 sm:mx-4 md:mx-6 lg:mx-8 xl:mx-14 2xl:mx-20 px-2 sm:px-3 md:px-4 lg:px-6 py-2.5 lg:py-3 md:gap-4">
          {/* Left: Logo & Location */}
          <div className="flex items-center justify-between w-full md:w-auto flex-shrink-0 gap-3 lg:gap-5">
            ...
            <div className="hidden sm:flex text-white items-center flex-shrink-0">
              <FontAwesomeIcon
                icon={faLocationDot}
                className="text-base lg:text-lg mr-1.5 lg:mr-2 text-gray-300"
              />
              <div className="font-semibold leading-tight">
                <h1 className="text-[11px] text-gray-400 font-normal">
                  {t("deliverTo")}
                </h1>
            ...
            <button
              className="md:hidden text-white text-2xl w-8 h-8 flex items-center justify-center outline-none"
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            >
              <FontAwesomeIcon
                icon={mobileMenuOpen ? faTimes : faBars}
                className={`transition-all duration-200 ${mobileMenuOpen ? "rotate-90 scale-110 text-orange-400" : "rotate-0 scale-100"}`}
              />
            </button>
          </div>

          {/* Mobile Menu Wrapper (Smooth animated transition) */}
          <div
            className={`w-full md:w-auto md:flex-1 transition-all duration-200 ease-in-out md:overflow-visible ${
              mobileMenuOpen
                ? "max-h-[600px] opacity-100"
                : "max-h-0 opacity-0 md:max-h-[600px] md:opacity-100"
            } ${menuOverflow === "hidden" ? "overflow-hidden" : "overflow-visible"}`}
          >
            <div className="flex flex-col md:flex-row md:items-center md:justify-between w-full pt-3 md:pt-0 pb-3 md:pb-0 gap-4 md:gap-0">
              {/* Center: Fluid Search Bar that fills available space evenly */}
              <div className="w-full md:flex md:flex-1 min-w-0 max-w-full md:max-w-xl lg:max-w-2xl 2xl:max-w-3xl mx-0 md:mx-2 lg:mx-4">
                <div className="flex w-full relative">
                  ...
                  <div className="relative flex-shrink-0" ref={catRef}>
                    ...
                    >
                      <span className="truncate max-w-[65px] lg:max-w-[85px]">
                        {tCategory(cat)}
                      </span>
                      <FontAwesomeIcon
                        icon={faChevronDown}
                      ...
                      <div className="absolute top-full left-0 right-0 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 z-50 max-h-60 overflow-auto shadow-lg rounded-b-md">
                        {suggestions.map((s, idx) => (
                          ...
                          >
                            <img
                              src={
                                s?.images[0] ? s?.images[0] : "/placeholder.png"
                              }
                              alt={s.name}
              ...
              <div className="w-full md:w-auto flex flex-col md:flex-row items-center md:space-x-2 lg:space-x-3 xl:space-x-4 2xl:space-x-5 space-y-3 md:space-y-0 flex-shrink-0">
                ...
                <div className="flex items-center space-x-2 lg:space-x-3.5 xl:space-x-4 2xl:space-x-5">
                  ...
                  <div className="relative" ref={langRef}>
                    ...
                    {openLang && (
                      <div className="absolute top-full left-0 md:left-auto md:right-0 mt-1 w-40 bg-white dark:bg-gray-800 rounded-md shadow-lg py-2 z-50">
                        {langs.map((l) => (
                          <button
                            key={l.code}
                  ...
                  >
                    <FontAwesomeIcon
                      ...
                      className="text-sm lg:text-base"
                    />
                    <h1 className="ml-1 font-medium text-xs lg:text-sm">
                      {t("cart")}
                    </h1>
                  </div>
                  ...
                  >
                    <span
                      className={`inline-block h-3.5 w-3.5 lg:h-4 lg:w-4 transform rounded-full bg-white dark:bg-gray-800 transition-transform duration-200 ${
                        isDarkMode
                          ? "translate-x-5 lg:translate-x-7"
                          : "translate-x-0.5 lg:translate-x-1"
                      }`}
                    />
                    <FontAwesomeIcon
                      icon={isDarkMode ? faMoon : faSun}
                      className={`absolute text-[10px] lg:text-xs ${
                        isDarkMode
                          ? "left-1 text-gray-300"
                          : "right-1 text-yellow-400"
                      }`}
                    />
                  </button>
                </div>
                ...
                <div className="pl-2 lg:pl-3 md:border-l border-gray-700 flex items-center">
                  {user ? (
                    ...
                    <Link to="/SignIn">
                      <div className="text-white cursor-pointer flex items-center hover:text-gray-300 transition px-2 py-1">
                        <FontAwesomeIcon
                          ...
                          className="text-sm lg:text-base"
                        />
                        <h1 className="ml-1.5 font-medium text-xs lg:text-sm">
                          {t("signIn")}
                        </h1>
                      </div>
                    </Link>
